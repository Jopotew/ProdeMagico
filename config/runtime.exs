...
#
#     PHX_SERVER=true bin/prode start
#
# Alternatively, you can use `mix phx.gen.release` to generate a `bin/server`
# script that automatically sets the env var above.
if System.get_env("PHX_SERVER") do
  config :prode, ProdeWeb.Endpoint, server: true
end

# Google OAuth credentials — read at runtime so they work in both dev (.env) and prod (Fly secrets)
if config_env() == :prod do
  config :ueberauth, Ueberauth.Strategy.Google.OAuth,
    client_id: System.get_env("GOOGLE_CLIENT_ID") || raise("GOOGLE_CLIENT_ID is not set"),
    client_secret:
...
if config_env() == :prod do
  ...
  config :prode, :dns_cluster_query, System.get_env("DNS_CLUSTER_QUERY")
